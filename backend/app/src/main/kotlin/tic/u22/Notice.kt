 ...
 * お知らせを削除する
 */
class DeleteNotice: RequestHandler<Map<String, Any>, String> {
    override fun handleRequest(event: Map<String, Any>?, context: Context?): String {
        val res = runBlocking {
            try {
                if (event == null) {
                ...
                val n_id = if (body["n_id"] != null) {body["n_id"]!! as String} else {throw Exception("n_id is null")}

                val dynamo = Dynamo(Settings().AWS_REGION)
                val tableName = "notice"

                val result = dynamo.deleteByKey(tableName, listOf(n_id))
