...
class Utils {
        ...
        val res =
                mp
                        .map {
                            if (!(it.key is String)) {
                                throw Exception(
                                        "key is not String while convert to AttributeValue.M"
                                )
                            }
    ...
    fun toAttributeValueMap(mp: Map<String, Any?>): Map<String, AttributeValue> {
        return mp.map { it.key to toAttributeValue(it.value) }.toMap()
    }
    ...
    /**
     * AttributeValueをTableBaseのデータクラスに変換する このメソッドでデータクラスに変換する際は、必ずデータクラスの変数の内容を含めること。
     * userデータクラスの場合、{"u_id": AttributeValue.S(u_id), ... , "authed":
     * AttributeValue.Bool(authed)}となる。
     *
     * @param attribute Map<String, AttributeValue>: AttributeValueのMap(データベースから取得したデータ)
     * @param tableType String: テーブル名( user | puzzle | book | notice | status | l_log | p_log |
     * b_log | sequence )
     *
     * return TableBase: 指定のデータクラスに変換されたオブジェクト
     */
    fun attributeValueToObject(
            attribute: Map<String, AttributeValue>,
            tableType: String
    ): TableBase {
        try {
            val values = attribute.map { it.key to toKotlinType(it.value) }.toMap()
            return when (tableType) {
                "user" ->
                        User(
                                u_id =
                                        if (values["u_id"] == null) {
                                            throw Exception("u_id is null")
                                        } else {
                                            if (values["u_id"]!! is String) {
                                                values["u_id"] as String
                                            } else {
                                                throw Exception("u_id type is ng")
                                            }
                                        },
                                family_name =
                                        if (values["family_name"] == null) {
                                            throw Exception("family_name is null")
                                        } else {
                                            if (values["family_name"]!! is String) {
                                                values["family_name"] as String
                                            } else {
                                                throw Exception("family_name type is ng")
                                            }
                                        },
                                first_name =
                                        if (values["first_name"] == null) {
                                            ...
                                            if (values["first_name"]!! is String) {
                                                values["first_name"] as String
                                            } else {
                                                throw Exception("first_name type is ng")
                                            }
                                        },
                                family_name_roma =
                                        if (values["family_name_roma"] == null) {
                                            throw Exception("family_name_roma is null")
                                        } else {
                                            if (values["family_name_roma"]!! is String) {
                                                values["family_name_roma"] as String
                                            } else {
                                                throw Exception("family_name_roma type is ng")
                                            }
                                        },
                                first_name_roma =
                                        if (values["first_name_roma"] == null) {
                                            throw Exception("first_name_roma is null")
                                        } else {
                                            if (values["first_name_roma"]!! is String) {
                                                values["first_name_roma"] as String
                                            } else {
                                                throw Exception("first_name_roma type is ng")
                                            }
                                        },
                                child_lock =
                                        if (values["child_lock"] == null) {
                                            throw Exception("child_lock is null")
                                        } else {
                                            if (values["child_lock"]!! is String) {
                                                values["child_lock"] as String
                                            } else {
                                                throw Exception("child_lock type is ng")
                                            }
                                        },
                                account_name =
                                        if (values["account_name"] == null) {
                                            throw Exception("account_name is null")
                                        } else {
                                            if (values["account_name"]!! is String) {
                                                values["account_name"] as String
                                            } else {
                                                throw Exception("account_name type is ng")
                                            }
                                        },
                                limit_time =
                                        if (values["limit_time"] == null) {
                                            throw Exception("limit_time is null")
                                        } else {
                                            if (values["limit_time"]!! is String) {
                                                (values["limit_time"] as String).toInt()
                                            } else {
                                                throw Exception("limit_time type is ng")
                                            }
                                        },
                                delete_flg =
                                        if (values["delete_flg"] == null) {
                                            throw Exception("delete_flg is null")
                                        } else {
                                            if (values["delete_flg"]!! is Boolean) {
                                                values["delete_flg"] as Boolean
                                            } else {
                                                throw Exception("delete_flg type is ng")
                                            }
                                        },
                        )
                "puzzle" ->
                        Puzzle(
                                p_id =
                                        if (values["p_id"] == null) {
                                            throw Exception("p_id is null")
                                        } else {
                                            if (values["p_id"]!! is String) {
                                                values["p_id"] as String
                                            } else {
                                                throw Exception("p_id type is ng")
                                            }
                                        },
                                title =
                                        if (values["title"] == null) {
                                            ...
                                            if (values["title"]!! is String) {
                                                values["title"] as String
                                            } else {
                                                throw Exception("title type is ng")
                                            }
                                        },
                                description =
                                        if (values["description"] == null) {
                                            ...
                                            if (values["description"]!! is String) {
                                                ...
                                                throw Exception("description type is ng")
                                            }
                                        },
                                icon =
                                        if (values["icon"] == null) {
                                            ...
                                            if (values["icon"]!! is String) {
                                                values["icon"] as String
                                            } else {
                                                throw Exception("icon type is ng")
                                            }
                                        },
                                words =
                                        if (values["words"] == null) {
                                            throw Exception("words is null")
                                        } else {
                                            if (values["words"]!! is List<Any?>) {
                                                                ...
                                                                it
                                                                        .map { item ->
                                                                            if (!(item.key is
                                                                                ...
                                                                                throw Exception(
                                                                                        "type of word (inside) is ng"
                                                                                )
                                                                            }
                                                                            (item.key as String) to
                                                                                    toAttributeValue(
                                                                                            item.value
                                                                                    )
                                                                        }
                                                ...
                                                throw Exception("words type is ng")
                                            }
                                        },
                                create_date =
                                        if (values["create_date"] == null) {
                                            throw Exception("create_date is null")
                                        } else {
                                            if (values["create_date"]!! is String) {
                                                values["create_date"] as String
                                            } else {
                                                throw Exception("create_date type is ng")
                                            }
                                        },
                                update_date =
                                        if (values["update_date"] == null) {
                                            throw Exception("update_date is null")
                                        } else {
                                            if (values["update_date"]!! is String) {
                                                values["update_date"] as String
                                            } else {
                                                throw Exception("update_date type is ng")
                                            }
                                        }
                        )
                "word" ->
                        Word(
                                word =
                                        if (values["word"] == null) {
                                            ...
                                            if (values["word"]!! is String) {
                                                values["word"] as String
                                            } else {
                                                throw Exception("word type is ng")
                                            }
                                        },
                                shadow =
                                        if (values["shadow"] == null) {
                                            throw Exception("shadow is null")
                                        } else {
                                            if (values["shadow"]!! is String) {
                                                values["shadow"] as String
                                            } else {
                                                throw Exception("shadow type is ng")
                                            }
                                        },
                                illustration =
                                        if (values["illustration"] == null) {
                                            throw Exception("illustration is null")
                                        } else {
                                            if (values["illustration"]!! is String) {
                                                values["illustration"] as String
                                            } else {
                                                throw Exception("illustration type is ng")
                                            }
                                        },
                                voice =
                                        if (values["voice"] == null) {
                                            throw Exception("voice is null")
                                        } else {
                                            if (values["voice"]!! is String) {
                                                values["voice"] as String
                                            } else {
                                                throw Exception("voice type is ng")
                                            }
                                        },
                                is_displayed =
                                        if (values["is_displayed"] == null) {
                                            throw Exception("is_displayed is null")
                                        } else {
                                            if (values["is_displayed"]!! is Boolean) {
                                                values["is_displayed"] as Boolean
                                            } else {
                                                throw Exception("is_displayed type is ng")
                                            }
                                        },
                                is_dummy =
                                        if (values["is_dummy"] == null) {
                                            throw Exception("is_dummy is null")
                                        } else {
                                            if (values["is_dummy"]!! is Boolean) {
                                                values["is_dummy"] as Boolean
                                            } else {
                                                throw Exception("is_dummy type is ng")
                                            }
                                        },
                        )
                "book" ->
                        Book(
                                b_id =
                                        if (values["b_id"] == null) {
                                            throw Exception("b_id is null")
                                        } else {
                                            if (values["b_id"]!! is String) {
                                                values["b_id"] as String
                                            } else {
                                                throw Exception("b_id type is ng")
                                            }
                                        },
                                title_jp =
                                        if (values["title_jp"] == null) {
                                            throw Exception("title_jp is null")
                                        } else {
                                            if (values["title_jp"]!! is String) {
                                                values["title_jp"] as String
                                            } else {
                                                throw Exception("title_jp type is ng")
                                            }
                                        },
                                title_en =
                                        if (values["title_en"] == null) {
                                            throw Exception("title_en is null")
                                        } else {
                                            if (values["title_en"]!! is String) {
                                                values["title_en"] as String
                                            } else {
                                                throw Exception("title_en type is ng")
                                            }
                                        },
                                summary =
                                        if (values["summary"] == null) {
                                            ...
                                            if (values["summary"]!! is String) {
                                                values["summary"] as String
                                            } else {
                                                throw Exception("summary type is ng")
                                            }
                                        },
                                author =
                                        if (values["author"] == null) {
                                            ...
                                            if (values["author"]!! is String) {
                                                values["author"] as String
                                            } else {
                                                throw Exception("author type is ng")
                                            }
                                        },
                                thumbnail =
                                        if (values["thumbnail"] == null) {
                                            ...
                                            if (values["thumbnail"]!! is String) {
                                                values["thumbnail"] as String
                                            } else {
                                                throw Exception("thumbnail type is ng")
                                            }
                                        },
                                pdf =
                                        if (values["pdf"] == null) {
                                            ...
                                            if (values["pdf"]!! is String) {
                                                values["pdf"] as String
                                            } else {
                                                throw Exception("pdf type is ng")
                                            }
                                        },
                                voice_keys =
                                        if (values["voice_keys"] == null) {
                                            throw Exception("voice_keys is null")
                                        } else {
                                            if (values["voice_keys"]!! is List<Any?>) {
                                                (values["voice_keys"] as List<Any?>).map {
                                                    if (it is String) {
                                                        it
                                                    } else {
                                                        throw Exception("voice_keys type is ng")
                                                    }
                                                }
                                            } else {
                                                throw Exception("voice_keys type is ng")
                ...
                "notice" ->
                        Notice(
                                n_id =
                                        if (values["n_id"] == null) {
                                            throw Exception("n_id is null")
                                        } else {
                                            if (values["n_id"]!! is String) {
                                                values["n_id"] as String
                                            } else {
                                                throw Exception("n_id type is ng")
                                            }
                                        },
                                title =
                                        if (values["title"] == null) {
                                ...
                                content =
                                        if (values["content"] == null) {
                                            ...
                                            if (values["content"]!! is String) {
                                                values["content"] as String
                                            } else {
                                                throw Exception("content type is ng")
                                            }
                                        },
                                create_date =
                                        if (values["create_date"] == null) {
                ...
                "status" ->
                        Status(
                                ...
                                game_status =
                                        if (values["game_status"] == null) {
                                            throw Exception("game_status is null")
                                        } else {
                                            if (values["game_status"]!! is String) {
                                                (values["game_status"] as String).toInt()
                                            } else {
                                                throw Exception("game_status type is ng")
                                            }
                                        },
                                status_infos =
                                        if (values["status_infos"] == null) {
                                            null
                                        } else {
                                            if (values["status_infos"]!! is List<Any?>) {
                                                (values["status_infos"] as List<Any?>).map {
                                                    if (it is String) {
                                                        it
                                                    } else {
                                                        throw Exception("status_infos type is ng")
                                                    }
                                                }
                                            } else {
                                                throw Exception("status_infos type is ng")
                        ...
                        LoginLog(
                                ...
                                datetime =
                                        if (values["datetime"] == null) {
                                            ...
                                            if (values["datetime"]!! is String) {
                                                values["datetime"] as String
                                            } else {
                                                throw Exception("datetime type is ng")
                                            }
                                        }
                        )
                "p_log" ->
                        PuzzleLog(
                                ...
                                play_times =
                                        if (values["play_times"] == null) {
                                            throw Exception("play_times is null")
                                        } else {
                                            if (values["play_times"]!! is String) {
                                                (values["play_times"] as String).toInt()
                                            } else {
                                                throw Exception("play_times type is ng")
                                            }
                                        },
                                latest_play_datetime =
                                        if (values["latest_play_datetime"] == null) {
                                            throw Exception("latest_play_datetime is null")
                                        } else {
                                            if (values["latest_play_datetime"]!! is String) {
                                                values["latest_play_datetime"] as String
                                            } else {
                                                throw Exception("latest_play_datetime type is ng")
                                            }
                                        }
                        )
                "b_log" ->
                ...
                "sequence" ->
                        Sequence(
                                tablename =
                                        if (values["tablename"] == null) {
                                            throw Exception("tablename is null")
                                        } else {
                                            if (values["tablename"]!! is String) {
                                                values["tablename"] as String
                                            } else {
                                                throw Exception("tablename type is ng")
                                            }
                                        },
                                now_seq =
                                        if (values["now_seq"] == null) {
                                            throw Exception("now_seq is null")
                                        } else {
                                            if (values["now_seq"]!! is String) {
                                                (values["now_seq"] as String).toInt()
                                            } else {
                                                throw Exception("now_seq type is ng")
                                            }
                                        }
                        )
...
val tableNameToKey: Map<String, List<String>> =
        mapOf(
                "user" to listOf("u_id"),
...
/**
 * Puzzleデータクラス(テーブル:puzzle) Listの部分は、mutableListOf()を使用すること
 *
 * @param p_id: String パズルのID(^p[0-9]{4}$)
 * @param title: String タイトル
 * @param description: String 概要
 * @param icon: String アイコン(default有-設定不要)
 * @param words: List<List<String>> 単語(正解順に格納・[[単語, 形状のS3キー, イラストのS3キー, 音声のS3キー]*単語数])([ [word,
 * ${Settings().AWS_BUCKET}/puzzle/shape/<ファイル名>, ${Settings().AWS_BUCKET}/puzzle/image/<ファイル名>,
 * ${Settings().AWS_BUCKET}/puzzle/voice/<ファイル名>], ])
...
/**
 * Bookデータクラス(テーブル:book) Listの部分は、mutableListOf()を使用すること
 *
...
/**
 * Status(テーブル:status) Listの部分は、mutableListOf()を使用すること
 *
 * @param u_id: String ユーザーのID
 ...
 */
data class Sequence(val tablename: String, val now_seq: Int) : TableBase
...
/**
 * statusテーブルのstatus_infosへの設定に利用 TODO: 仕様変更に対応して変える generateDbForm: フロント形式 -> DB形式
