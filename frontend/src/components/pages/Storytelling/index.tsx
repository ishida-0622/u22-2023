...
import { StartBookRequest, StartBookResponse } from "@/features/book/types/start";
    ...
    return (
        <div>
            {/* 絵本を置く場所 */}
                ...
                <button id="previousPage" className={currentPage == 1 ? styles.previousButtonInvalid : styles.previousButton} onClick={() => previousPage()}>←</button>
                <div className={styles.book}>
                <Document file={currentPageUrl} >
                <Page width={300} pageNumber={currentPage} />
                </Document>
                </div>
                <button id="nextPage" className={currentPage == maxPages ? styles.nextButtonInvalid : styles.nextButton} onClick={() => nextPage()}>→</button>
            {/* 朗読を始めるボタン */}
            <AudioPlayer vol={volume} audio={currentAudio}/>
            <input type="range" min="0.0" max="1.0" step="0.1" value={volume} onChange={(event) => setVolume(Number(event.target.value))}></input>
